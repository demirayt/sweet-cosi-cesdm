...
ENERGY_CARRIER_PRICE = {
    "carrier.fuel.fossil.gas.natural_gas": 22.642024452658500,
    "carrier.fuel.fossil.coal.hard_coal": 6.396724667349030,
    "carrier.fuel.fossil.coal.lignite": 7.18,
    "carrier.fuel.fossil.oil": 33.026621160409600,
    "carrier.fuel.fossil.oil.shale": 6.696000000000000,
    "carrier.fuel.biofuel.biomass": 67.6936567357147,
    "carrier.hydrogen": 54.432000000000000,
    "carrier.fuel.nuclear.uranium": 6.051054545454550,
    "carrier.resource.renewable.solar": 0.0,
    "carrier.resource.renewable.wind": 0.0,
    "carrier.resource.water": 0.0,
    "carrier.electricity": 0.0
}

ENERGY_CARRIER_CO2  = {
    "carrier.fuel.fossil.gas.natural_gas": 0.203,
    "carrier.fuel.fossil.coal.hard_coal": 0.340,
    "carrier.fuel.fossil.coal.lignite": 0.363,
    "carrier.fuel.fossil.oil": 0.280,
    "carrier.fuel.fossil.oil.shale": 0.359,
...
ENERGY_CARRIER_MAP  = {
    ...
    "solar": "carrier.resource.renewable.solar",
    "wind": "carrier.resource.renewable.wind",
    "water": "carrier.resource.water",
...
def assign_installed_capacity_from_tyndp_csv(
    ...
    *,
    ...
    def ensure_carrier(carrier_name: str) -> str:
        cid = f"{ENERGY_CARRIER_MAP[_slug(carrier_name)]}"
...
def assign_ntc_from_tyndp_ntc_types_base_csv(
    ...
    *,
    ...
    for _, row in df_final.iterrows():
        ...
        if "EnergyNode" in model.entities:
            if from_node_id not in model.entities["EnergyNode"]:
                continue
            if to_node_id not in model.entities["EnergyNode"]:
                continue
...
def build_cesdm_model_from_tyndp_installed_capacities(
    ...
    *,
    ...
    model.export_yaml(scenario_output_folder + "/cesdm/yaml/" + yaml_file_name + ".yaml")
    # model.export_datapackage(scenario_output_folder + "/cesdm/package/")
    model.export_csv_by_class_wide(scenario_output_folder + "/cesdm/tableview/")
    # model.export_long_csv(scenario_output_folder + "/cesdm/tyndp_rowbased.csv")

    save_timeseries_to_hdf5(filename=str(scenario_output_folder + "/cesdm/yaml/profile_data.h5"), timestamps=None, data_dict=profiles)
...
def import_cesdm_model_from_tyndp_installed_capacities(
    ...
    *,
    ...
    model.import_csv_by_class_wide(scenario_output_folder + "/cesdm/tableview/")
    ...
    # model.export_yaml(scenario_output_folder + "/cesdm/yaml/" + yaml_file_name + "_v2.yaml")
